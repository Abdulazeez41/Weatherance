...
const deployContract = async function (value = ethers.utils.parseEther("0.1")) {
  const Weatherance = await ethers.getContractFactory("Weatherance");
  const weatherance = await Weatherance.deploy({ value: value });
  return await weatherance.deployed();
};

describe("Weatherance", function () {
  it("Should have balance used to initialize the contract", async function () {
    const value = ethers.utils.parseEther("1.0");
    const weatherance = await deployContract(value);
    const provider = ethers.provider;
    const initialBalance = await provider.getBalance(weatherance.address);

    expect(initialBalance).to.equal(value);
  });

  it("Should shift temperatures", async function () {
    const weatherance = await deployContract();
    ...
    const fakeInsuranceAlice = await weatherance.createInsurance(
      alice.address,
      premium,
      30,
      25,
    ...
    const updatedFakeInsuranceAlice = await weatherance.shiftTemperatures(
      fakeInsuranceAlice,
      35
    );
  ...
  it("Should evaluate possible payments based on last five days temperature", async function () {
    ...
    expect(
      await weatherance.shouldPaySettlement(fakeInsuranceAlice)
    ).be.equal(false);

    // it's fake because it's just a mock object, not on the chain
    const fakeInsuranceBob = await weatherance.createInsurance(
      bob.address,
      premium,
      40,
      41,
    ...
    expect(
      await weatherance.shouldPaySettlement(fakeInsuranceBob)
    ).be.equal(true);
  });
  ...
  it("Should pay settlement", async function () {
    ...
    const aliceConnectedToWeatherContract = weatherance.connect(alice);
    ...
    const insuranceAlice = await weatherance.insurances(0);
    await weatherance.paySettlement(insuranceAlice);
    ...
    const multiplier = await weatherance.SETTLEMENT_MULTIPLIER();
  ...
  it("Should update temperature for all insurances", async function () {
    ...
    const bobConnectedToWeatherContract = weatherance.connect(bob);
    await bobConnectedToWeatherContract.buyInsurance(optionsBob);

    await weatherance.updateTemperature(25);

    const insuranceAlice = await weatherance.insurances(0);
    const insuranceBob = await weatherance.insurances(1);
...
describe("Weather Insurance contract user", function () {
  it("Should be able to buy an insurance", async function () {
    ...
    const insurance = await weatherance.insurances(0);
